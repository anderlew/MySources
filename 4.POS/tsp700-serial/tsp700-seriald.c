...
FILE *openOutFile() {
	...
	fileName = getFileName(NULL, NULL, NULL);
	if (NULL == fileName) {
		return NULL;
	}
...
void testWriteSerial(int devFD)
{
	char buff[1024] = {0};

	while (1) {
		while ((read(devFD, buff, 1024)) > 0) {
			printf("%s", buff);
		}
	}
...
int main(int argc, char* argv[])
{
	...
	if (setSpeed(devFD, SPEED_INT_LIST[0]) == -1) {
